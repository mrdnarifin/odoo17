...
class HrEmployeePrivate(models.Model):
    ...
    private_country_id = fields.Many2one(
        "res.country", string="Private Country", groups="hr.group_hr_user"
    )
    ...
    work_permit_name = fields.Char(
        "work_permit_name", compute="_compute_work_permit_name"
    )
    ...
    def _check_private_fields(self, field_names):
        ...
        if private_fields:
            raise AccessError(
                _(
                    "The fields %r you try to read is not available on the public employee profile.",
                    ",".join(private_fields),
                )
            )
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        return super(HrEmployeePrivate, self.sudo())._search(
            [("id", "in", ids)], order=order
        )
