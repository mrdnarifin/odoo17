...
class HrAttendance(models.Model):
    ...
    check_in = fields.Datetime(
        string="Check In", default=fields.Datetime.now, required=True, tracking=True
    )
    ...
    in_country_name = fields.Char(
        string="Country", help="Based on IP Address", readonly=True
    )
            ...
            for row in a:
                if row["ot_id"] and row["att_wh"]:
                    if row["ot_id"] not in grouped_dict:
                        grouped_dict[row["ot_id"]] = {
                            "attendances": [(row["att_id"], row["att_wh"])],
                            "overtime_duration": row["ot_d"],
                        }
                    else:
                        grouped_dict[row["ot_id"]]["attendances"].append(
                            (row["att_id"], row["att_wh"])
                        )
                ...
                for att in grouped_dict[ot]["attendances"]:
                    if ot_bucket > 0:
                        ...
                        if sub_time < 0:
                            ...
                            att_progress_values[att[0]] = float(
                                ((att[1] - ot_bucket) / att[1]) * 100
                            )
                            ot_bucket = 0
                    else:
                    ...
                    # The employee usually work on that day
                        ...
                        for attendance in attendances:
                            ...
                            if (
                                ...
                                lunch_intervals = calendar._attendance_intervals_batch(
                                    start_dt, stop_dt, emp.resource_id, lunch=True
                                )
                                work_duration -= sum(
                                    (i[1] - i[0]).total_seconds() / 3600.0
                                    for i in lunch_intervals[emp.resource_id.id]
                                )
